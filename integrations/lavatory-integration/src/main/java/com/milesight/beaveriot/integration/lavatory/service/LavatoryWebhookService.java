...
@Slf4j
@Service
public class LavatoryWebhookService {

    private static final String WEBHOOK_STATUS_KEY = LavatoryConnectionPropertiesEntities.getKey(LavatoryConnectionPropertiesEntities.Fields.webhookStatus);
    ...
    @Autowired
    private LavatoryDataSyncService dataSyncService;

    public void init() {
        val webhookSettingsKey = LavatoryConnectionPropertiesEntities.getKey(LavatoryConnectionPropertiesEntities.Fields.webhook);
        val webhookSettings = entityValueServiceProvider.findValuesByKey(webhookSettingsKey, LavatoryConnectionPropertiesEntities.Webhook.class);
    ...
    @EventSubscribe(payloadKeyExpression = "lavatory.integration.webhook.*", eventType = ExchangeEvent.EventType.DOWN)
    public void onWebhookPropertiesUpdate(Event<LavatoryConnectionPropertiesEntities.Webhook> event) {
    ...
    private void handleDeviceData(WebhookPayload webhookPayload) {
        ...
        val client = lavatoryClientProvider.getMscClient();
        ...
        val device = deviceServiceProvider.findByIdentifier(sn, LavatoryIntegrationConstants.INTEGRATION_IDENTIFIER);
        if (device == null) {
            log.warn("Device not added, try to sync data: {}", sn);
            dataSyncService.syncDeviceData(new com.milesight.beaveriot.integration.lavatory.service.LavatoryDataSyncService.Task(com.milesight.beaveriot.integration.lavatory.service.LavatoryDataSyncService.Task.Type.ADD_LOCAL_DEVICE, sn, null));
            return;
        }
