...
@Getter
@Service
public class LavatoryDataSyncService {
    ...
    @EventSubscribe(payloadKeyExpression = "lavatory.integration.scheduled_data_fetch.*", eventType = ExchangeEvent.EventType.DOWN)
    public void onScheduledDataFetchPropertiesUpdate(Event<LavatoryConnectionPropertiesEntities.ScheduledDataFetch> event) {
    ...
    @EventSubscribe(payloadKeyExpression = "lavatory.integration.openapi_status", eventType = ExchangeEvent.EventType.DOWN)
    public void onOpenapiStatusUpdate(Event<LavatoryConnectionPropertiesEntities> event) {
    ...
    @SneakyThrows
    @EventSubscribe(payloadKeyExpression = "lavatory.integration.sync_device", eventType = ExchangeEvent.EventType.DOWN)
    public void onSyncDevice(Event<LavatoryServiceEntities.SyncDevice> event) {
    ...
    public void start() {
        ...
        if (periodSeconds == 0) {
            val scheduledDataFetchSettings = entityValueServiceProvider.findValuesByKey(
                    LavatoryConnectionPropertiesEntities.getKey(LavatoryConnectionPropertiesEntities.Fields.scheduledDataFetch),
                    LavatoryConnectionPropertiesEntities.ScheduledDataFetch.class);
    ...
    private void syncDevicesFromMsc() throws IOException {
        ...
        val allDevices = deviceServiceProvider.findAll(LavatoryIntegrationConstants.INTEGRATION_IDENTIFIER);
    ...
    private int getAndUpdateLastSyncTime(Device device) {
        ...
        val lastSyncTimeKey = LavatoryIntegrationConstants.InternalPropertyIdentifier.getLastSyncTimeKey(device.getKey());
    ...
    private void syncPropertiesHistory(Device device, int lastSyncTime) {
        ...
        while (hasNextPage) {
            val page = lavatoryClientProvider.getMscClient()
                    .device()
    ...
    public void saveHistoryData(String deviceKey, String eventId, JsonNode data, long timestampMs, boolean isLatestData) {
        val payload = eventId == null
                ? LavatoryTslUtils.convertJsonNodeToExchangePayload(deviceKey, data)
                : LavatoryTslUtils.convertJsonNodeToExchangePayload(String.format("%s.%s", deviceKey, eventId), data, false);
    ...
    private Device updateLocalDevice(Task task) {
        ...
        val thingSpec = lavatoryDeviceService.getThingSpec(String.valueOf(deviceId));
        return lavatoryDeviceService.updateLocalDevice(task.identifier, String.valueOf(deviceId), thingSpec);
    }
    ...
    private Device addLocalDevice(Task task) {
        ...
        val deviceId = details.getDeviceId();
        val thingSpec = lavatoryDeviceService.getThingSpec(String.valueOf(deviceId));
        return lavatoryDeviceService.addLocalDevice(task.identifier, details.getName(), 0, String.valueOf(deviceId), thingSpec);
    }
    ...
    private DeviceDetailResponse getDeviceDetails(Task task)
        ...
        if (details == null) {
            details = lavatoryClientProvider.getMscClient().device().searchDetails(DeviceSearchRequest.builder()
                            .sn(task.identifier)
                            .pageNumber(1L)
                            .pageSize(1L)
                            .build())
